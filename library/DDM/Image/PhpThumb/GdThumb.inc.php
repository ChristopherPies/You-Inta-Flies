...
class GdThumb extends ThumbBase
{
	...
	/**
	 * The requested dimensions of the image
	 *
	 * @var array
	 */
	protected $requestedDimensions;
	/**
	...
	public function resize ($maxWidth = 0, $maxHeight = 0)
	{
		...
		$this->setRequestedDimensions( array('width' => $maxWidth, 'height' => $requestedHeight) );

		// get the new dimensions...
		$this->calcImageSize($this->currentDimensions['width'], $this->currentDimensions['height']);

		// set the requested height if it was missing.
		if($this->requestedDimensions['height'] == 0 && $this->newDimensions['newHeight']) {
		   $this->requestedDimensions['height'] = round($this->requestedDimensions['width'] * $this->newDimensions['newWidth'] / $this->newDimensions['newHeight'] );
		}

		if( $maxHeight == 0 && $maxWidth > 0 ) {
			$maxHeight = round( $this->currentDimensions['height'] * $maxWidth / $this->currentDimensions['width']);
		}

		if( $maxWidth == 0 && $maxHeight > 0 ) {
			$maxWidth = round( $this->currentDimensions['width'] * $maxHeight / $this->currentDimensions['height']);
		}
	 ...
	 *
	 * @param string $fileName The full path and filename of the image to save (optional, will append size to src name)
	 * @param string $format The format to save the image in (optional, must be one of [GIF,JPG,PNG]
	 * @param string $append The string to append to the filename (if null fileName is passed) when saving the new image
	 * @return GdThumb
	 */
	public function save ($fileName = null, $format = null, $append = null)
	{
		...
		// append the size if a name was not supplied
		if( $fileName === null ) {
                        if($append === null)
                        {
                            $append = $this->requestedDimensions['width'] . 'x'. $this->requestedDimensions['height'];
                        }
			$tmpName = $this->fileName;
			$tmp = pathinfo($tmpName);
			$fileName = $tmp['dirname'] . '/' . $tmp['filename'] . '_'. $append . '.' . $tmp['extension'];
		}
	...
	/**
	 * Fix rotation using exif info
	 *
	 */
	public function fixRotation( ) {

		if( !function_exists('exif_read_data') ) {
			throw new RuntimeException ('Exif not installed');
		}
	...
	/**
	 * Returns $requestedDimensions.
	 *
	 * @see GdThumb::$requestedDimensions
	 */
	public function getRequestedDimensions ()
	{
	...
	/**
	 * Sets $requestedDimensions.
	 *
	 * @param array $requestedDimensions
	 * @see GdThumb::$requestedDimensions
	 */
	public function setRequestedDimensions ($requestedDimensions)
	{
